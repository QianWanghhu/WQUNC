...
# Calculate the contribution of each component model to event load mass.
# Read the data containing contribution of each parameter
contri_data = pd.read_csv(f'{outpath}contribution_each_param.csv', index_col='Date')
contri_event = pd.DataFrame(index=events.index, columns=contri_data.columns)
contri_data.index = pd.to_datetime(contri_data.index)
# Extract data for each event according to the start and end time.
for ii in range(1, events.shape[0]+1):
    t_start, t_end = events.loc[ii, 'start':'end']
    df_temp = contri_data.loc[t_start:t_end, :]
    contri_event.loc[ii, :] = df_temp.sum(axis=0) / df_temp.sum(axis=0)[0]
contri_event.to_csv(f'{outpath}contribution_each_event.csv')
